(() => {
  ...
  const bot = {
    ...
    armedAtId: 0,
    lastPhase: "unknown",
    siteMessage: "Moon Row bot loaded. Open popup and Start.",
    logs: [],
    betLog: [],
    lastPlaceFail: 0,
  ...
  function die() {
    ...
    window.__bcMoonRowLoaded = false;
  }
  ...
  function syncPendingFlag() {
    try {
      const on = !!(bot.pending && !bot.pending.sawRound);
  ...
  function observeRound(value, id) {
    ...
    engine.onRoundObserved(value, id);
    log(engine.state.message);
    if (engine.shouldBet() && !bot.pending && !bot.placing) {
      bot.awaitingBet = true;
      bot.armedAtId = engine.armAfterId() || id || bot.lastGameId;
      syncAwaitingFlag();
      startPlacePump();
      tryPlace();
    } else if (!engine.shouldBet()) {
  ...
  function armBet(id) {
    bot.awaitingBet = true;
    bot.armedAtId = id || engine.armAfterId() || bot.lastGameId;
    syncAwaitingFlag();
    startPlacePump();
    tryPlace();
  }
  ...
  function startPlacePump() {
    ...
    bot.placePump = setInterval(() => {
      if (!extAlive() || !bot.betting || !bot.awaitingBet || bot.pending || bot.placing) {
        if (!bot.awaitingBet || bot.pending || bot.placing || !bot.betting) {
          if (bot.placePump) {
            clearInterval(bot.placePump);
            bot.placePump = null;
          }
  ...
  function canPlace() {
    ...
    if (engine.state.mode === MoonMode.STOPPED) return false;
    if (!engine.shouldBet()) return false;
    if (!bot.lastGameId) return false;
    if (Date.now() < (bot.placeAfter || 0)) return false;
    if (Date.now() - bot.lastPlaceFail <= 120) return false;
    const games = collectBannerGames();
    const armId = bot.armedAtId || engine.armAfterId();
    if (games.length && armId && games[games.length - 1].id > armId) return false;
    return betButtonReady();
  }

  function tryPlace() {
    if (!canPlace()) return;
    if (engine.state.mode === MoonMode.STOPPED) {
      bot.betting = false;
      syncBettingFlag();
      log(engine.state.message);
      return;
    ...
    absorbVisibleBanner();
    const lockId = bot.armedAtId || engine.armAfterId() || bot.lastGameId;
    if (!lockId) return;
    bot.placing = true;
    bot.pending = {
      stake: nxt.stake,
  ...
  function settle() {
    ...
    bot.settledGameId = crashId || 0;
    bot.placeAfter = Date.now() + (won ? 80 : 0);
    bot.armedAtId = 0;
    syncPendingFlag();
    if (engine.shouldBet()) {
      bot.awaitingBet = true;
      bot.armedAtId = engine.armAfterId();
      syncAwaitingFlag();
      startPlacePump();
    }
    if (engine.state.mode === MoonMode.STOPPED) {
  ...
  function applyBannerReplay() {
    const games = collectBannerGames();
    if (games.length) {
      const newest = games[games.length - 1].id;
      if (!bot.lastGameId || newest > bot.lastGameId) bot.lastGameId = newest;
    }
    engine.replayGames(games.map((g) => ({ id: g.id, value: g.v })));
    log(engine.state.message);
  }
  ...
  function onKeepaliveTick() {
    ...
    if (bot.betting && (bot.awaitingBet || bot.pending)) tryPlace();
  ...
  function renderOverlay() {
    let el = document.getElementById("bc-moon-row-overlay");
    if (!el) {
      el = document.createElement("div");
      el.id = "bc-moon-row-overlay";
      document.documentElement.appendChild(el);
    }
    const s = engine.snapshot();
    const live = engine.state.last_multiplier_seen;
    el.innerHTML = `
      <div class="title">Moon Row Bot v${extVersion}</div>
      <div class="row"><span class="k">Betting</span><span class="v ${bot.betting ? "on" : "off"}">${bot.betting ? "ON" : "off"}</span></div>
      <div class="row"><span class="k">Row</span><span class="v">${s.row_type} pos ${s.row_pos}</span></div>
      <div class="row"><span class="k">Seq</span><span class="v">${s.sequence_attempt ? s.sequence_attempt + "/" + s.sequence_max : "—"}</span></div>
    ...
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      ...
      if (msg.type === "START_BET") {
        ...
        log("Betting ON — watching moon rows");
        if (engine.shouldBet()) armBet(engine.armAfterId());
        sendResponse(status());
        return;
      }
      ...
      if (msg.type === "UPDATE_CONFIG") {
        botConfig = { ...botConfig, ...(msg.config || {}) };
        engine.updateConfig(botConfig);
    ...
    loadBetLogs(() => {
      chrome.storage.local.get("config", (data) => {
        if (!extAlive()) return;
        if (data && data.config) {
          botConfig = { ...defaultMoonRowConfig(), ...data.config };
          engine.updateConfig(botConfig);
        }
        const bal = readBalance();
        if (bal != null) engine.setStartBalance(bal);
        renderOverlay();
        log("Ready — Moon Row on crash page. Click Start in popup.");
        startLiveUpdates();
      });
    });

    tickTimer = setInterval(tick, 120);
    window.__bcMoonRowLoaded = true;
  }

  try {
    boot();
  } catch (err) {
    console.error("[Moon Row] boot failed", err);
